...
function Reportes() {
    ...
    const esc = (v) => String(v ?? "")
        ...
        .replace(/"/g, "&quot;").replace(/'/g, "&apos;");
    // Encabezado -> nombre de etiqueta XML válido (sin acentos ni símbolos)
    const tag = (s) => {
        let t = String(s).normalize("NFD").replace(/[̀-ͯ]/g, "")
            .replace(/[^A-Za-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
        if (!t || /^[0-9]/.test(t)) t = "c_" + t;
        return t;
    };

    const exportarXML = () => {
        const meta = TIPOS.find(t => t.key === tipo)?.label || tipo;
        let cols = [], filas = [], itemTag = "item";
        ...
        if (tipo === "ventas") {
            ...
            ventasFiltradas.forEach(v => {
                ...
                filas.push([v.fechaVenta, `${v.serieComprobante}-${v.numeroComprobante}`, v.pasajeroNombre,
                    viaje?.rutaNombre || "", v.metodoPago || "", Number(v.descuento || 0).toFixed(2), Number(v.precio || 0).toFixed(2)]);
            });
        } else if (tipo === "pagos") {
            cols = ["Metodo", "Iquitos", "Requena", "Otro_Web", "Total", "Pasajes"];
            itemTag = "metodo";
            reportePagos.filasVisibles.forEach(f => {
                const r = reportePagos.m[f];
                filas.push([METODO_LABEL[f], r.IQUITOS.i.toFixed(2), r.REQUENA.i.toFixed(2), r.OTRO.i.toFixed(2), r.total.i.toFixed(2), r.total.c]);
            });
        } else if (tipo === "vendedores") {
            cols = ["Vendedor", "Pasajes", "Ingreso"];
            itemTag = "vendedor";
            rankingVendedores.forEach(r => filas.push([r.nombre, r.pasajes, r.ingreso.toFixed(2)]));
        } else if (tipo === "horas") {
            cols = ["Hora", "Pasajes", "Ingreso"];
            itemTag = "hora";
            ventasPorHora.filter(h => h.pasajes > 0).forEach(h => filas.push([h.hora, h.pasajes, h.ingreso.toFixed(2)]));
        } else if (tipo === "asientos") {
            cols = ["Tipo", "Pasajes", "Ingreso"];
            itemTag = "tipo";
            asientosTipo.forEach(a => filas.push([a.tipo, a.pasajes, a.ingreso.toFixed(2)]));
        } else if (tipo === "ocupacion") {
            cols = ["Codigo", "Ruta", "Fecha", "Hora", "Vendidos", "Capacidad", "Ocupacion_pct"];
            itemTag = "viaje";
            ...
            cols = ["Codigo", "Ruta", "Fecha", "Hora", "Estado"];
            itemTag = "viaje";
            viajesEnRango.forEach(v => filas.push([v.codigoViaje, v.rutaNombre, v.fechaSalida, v.horaSalida, v.estado]));
        }

        const tags = cols.map(tag);
        const cuerpo = filas.map(fila =>
            `    <${itemTag}>\n` +
            fila.map((val, i) => `      <${tags[i]}>${esc(val)}</${tags[i]}>`).join("\n") +
            `\n    </${itemTag}>`
        ).join("\n");

        const xml = `<?xml version="1.0" encoding="UTF-8"?>\n` +
            `<reporte tipo="${esc(tipo)}" titulo="${esc(meta)}" desde="${esc(desde)}" hasta="${esc(hasta)}" generado="${esc(new Date().toISOString())}">\n` +
            cuerpo + `\n</reporte>\n`;
        ...
        a.href = url;
        a.download = `reporte_${tipo}_${desde}_a_${hasta}.xml`;
        a.click();
        ...
        <div className="reportes-page">
            ...
            <div className="reportes-header">
                ...
                <div className="reportes-acciones">
                    <button className="btn-secundario" onClick={exportarXML}>
                        <i className="ti ti-download"></i> Exportar XML
                    </button>
